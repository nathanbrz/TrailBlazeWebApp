...
describe('Signup Integration Test', () => {
    ...
    // Test if error shows when numbers are used for first name
    it('should show an error message if first name includes numbers', async () => {

        doCreateUserWithEmailAndPassword.mockRejectedValueOnce(new Error("First name should contain only letters."));
        const { getByLabelText, getByText, getByRole } = render(<Signup />);
        
        
        // Using numbers for name
        fireEvent.change(getByLabelText(/first name/i), { target: { value: '123' } });
        fireEvent.change(getByLabelText(/last name/i), { target: { value: 'Doe' } });
        fireEvent.change(getByLabelText(/email/i), { target: { value: 'john.doe@example.com' } });
        fireEvent.change(getByLabelText(/password/i), { target: { value: 'password123' } });
     ...
     // Test if error shows when first name is left empty
     it('should show an error message when first name is empty', async () => {
       ...
       // Test if error shows when numbers are used for last name
       it('should show an error message when numbers are used for last name', async () => {

        doCreateUserWithEmailAndPassword.mockRejectedValueOnce(new Error("Last name should contain only letters."));
        const { getByLabelText, getByText, getByRole } = render(<Signup />);
        
        
        // Using numbers for name
        fireEvent.change(getByLabelText(/first name/i), { target: { value: 'Riley' } });
        fireEvent.change(getByLabelText(/last name/i), { target: { value: '123' } });
        fireEvent.change(getByLabelText(/email/i), { target: { value: 'john.doe@example.com' } });
        fireEvent.change(getByLabelText(/password/i), { target: { value: 'password123' } });
    ...
    // Test if error shows when last name is left empty
    ...
    // Test if error shows when email is left blank
    ...
    // Test if error shows when password is not long enough (length < 6)
    it('should show an error message for invalid password', async () => {
        const { getByLabelText, getByText, getByRole } = render(<Signup />);

        doCreateUserWithEmailAndPassword.mockRejectedValueOnce(new Error("Password should be at least 6 characters long."));
